...
<div class="card">
    <div class="card-body">
        <div class="panel-heading">
            <h6 class="panel-title mb-4">REKAP PENCATATAN PEMBERIAN BANTUAN - {{$anakasuh->nama}}</h6>
            @include('alert')
            <h9 class="panel-title" style="padding: 20px 20px 20px 0PX;"><b>Cari Berdasarkan Tanggal</b></h9>
            <div class="judul-search">
                <form class="form" action="{{url('/bantuan/rekappencatatan/'.$anakasuh->id)}}" method="GET">
            ...
            <table class="table table-bordered table-striped table-hover table-sm datatable">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{$tr->keterangan}}</td>
                        <td>{{$tr->oleh}}</td>
                        <td><a href="{{url('/bantuan/editpencatatan/'.$tr->id)}}" class="btn btn-primary btn-sm"><i class="la la-pencil"></i> </a></td>
                        <td>
                            <form action="{{url('/bantuan/pencatatan/'.$tr->id)}}" method="post" class="delete">
                                @method ('delete')
                                @csrf
